...
word representations for each sub-corpora. For some subcorpus $x\in\{1A, 1B, 1C, 2A, 2B\}$, we denote
...
\end{equation}
where $\mathcal{C}$ is the convex set of matrices consisting of vectors
with norm at most $1$ and $\alpha$ contains the sparse coefficients of word forms. The only difference
