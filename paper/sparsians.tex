...
In our official submission we regarded those hypernyms as potential candidates 
during test time which occurred at least once as a correct hypernym during 
training. We call this strategy as candidate filtration. Since the potential 
set of hypernyms is much wider than that, we naturally could not achieve 
perfect score on the test set as a consequence. \autoref{table:upper} contains 
the best possible metrics on the test set that we could achieve as a result of 
candidate filtration.

\input{upper}


Based on our post evaluation experiments

\input{post_eval}

%\subsection{Candidate restriction}
