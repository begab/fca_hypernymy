...
\begin{table}
    ...
    child  & $n(h)\prec n(q)$ \\
    \midrule 
    frequent & see the footnote\footnotemark \\
    \bottomrule
     a beküldés pillanatában nem úgy működött mint ahogy kellett volna neki)
	\end{tabular}
  \caption{The feature in our classifier.}
...
At the time of submission, this feature did not work the way it should have
done.}
...
Additionally, we refer to the set of basis vectors (in the FCA terminology,
attributes) which are assigned nonzero
...
% specific)
$n(w)$ in \autoref{tab:core_features} denotes the node in the formal concept
...
Our most important features are \texttt{attributePair}$_{ij}$s above are that
are indicator features for every possible
interaction term
