...
if __name__=="__main__":
        ...
        dataset_mapping = {'1A':'english','1B':'italian','1C':'spanish','2A':'medical','2B':'music'}

        if len(sys.argv) == 1:
            print("missing command line parameter specifying the alphas file")
            sys.exit(2)
        else:
            alphas_file = sys.argv[1]
        dataset_id = alphas_file[0:2]


        possible_hypernyms_file = '/home/berend/datasets/semeval2018/SemEval18-Task9/vocabulary/{}.{}.vocabulary.txt'.format(
                    dataset_id, dataset_mapping[dataset_id])
        data_file = '/home/berend/datasets/semeval2018/SemEval18-Task9/training/data/{}.{}.training.data.txt'.format(
                    dataset_id, dataset_mapping[dataset_id])
        gold_file = '/home/berend/datasets/semeval2018/SemEval18-Task9/training/gold/{}.{}.training.gold.txt'.format(
                    dataset_id, dataset_mapping[dataset_id])

        possible_hypernyms = [l.strip().replace(' ', '_')
                          for l in open(possible_hypernyms_file, 'r')]
        train_queries = [l.split('\t')[0].replace(' ', '_') for l in open(data_file)]
        train_golds = [ x.replace(' ', '_')
            for line in open(gold_file) for x in line.strip().split('\t')
        ]

        dev_queries = [l.split('\t')[0].replace(' ', '_') for l in open(data_file.replace('training', 'trial'))]
        dev_golds = [
            x.replace(' ', '_')
            for line in open(gold_file.replace('training', 'trial')) for x in line.strip().split('\t')
        ]
        test_queries = [l.split('\t')[0].replace(' ', '_') for l in open(data_file.replace('training', 'test'))]
        should_have = set(train_queries) | set(train_golds) | set(dev_queries) | set(dev_golds) | set(test_queries)

        vocab = [l.strip() for l in open('{}.vocab'.format(dataset_id))]
        word_frequencies = {l.split('\t')[0].replace(' ', '_'): int(l.split('\t')[1])
                           for l in open('/home/berend/datasets/semeval2018/SemEval18-Task9/SemEval2018_Frequency_lists/{}_{}_frequencylist.txt'.format(dataset_id, dataset_mapping[dataset_id]))}
        alphas = pickle.load(open(alphas_file, 'rb'))
        print(dataset_id, len(vocab), alphas.shape, alphas.nnz, 100.*alphas.nnz / np.product(alphas.shape))

        relation=[]
        words_discarded = 0
        included_words = set()
        for i, w in enumerate(vocab):
            coeffs = alphas[:, i].indices
            if w not in should_have and (len(coeffs) == 0 or (w in word_frequencies and word_frequencies[w] < 25)):
                words_discarded += 1
                continue
            included_words.add(w)
            for c in coeffs:
                relation+=[(w,'n{}'.format(c))]
            if i%12500 == 0:
                print(i)
        print(len(included_words), len(included_words & should_have), len(should_have))
        print(words_discarded, len(relation), alphas.shape[1]-words_discarded)
        ...
        dotfile=open("{}_more_permissive.dot".format(alphas_file),"w")
